...
public class GestureDetector : IDisposable
{
    ...
    private readonly string handsDB = "GestureDB\\LeftAndRightHandsSolution.gbd";


    /// <summary> Name of the discrete gesture in the database that we want to track </summary>
    private readonly string handsLeftGestureName = "Hands_Left";
    private readonly string handsRightGestureName = "Hands_Right";
    ...
    public GestureDetector(KinectSensor kinectSensor)
    {
        ...
        // load the 'Seated' gesture from the gesture database
        var databasePath = Path.Combine(Application.streamingAssetsPath, this.handsDB);
        using (VisualGestureBuilderDatabase database = VisualGestureBuilderDatabase.Create(databasePath))
        {
            // we could load all available gestures in the database with a call to vgbFrameSource.AddGestures(database.AvailableGestures), 
            // but for this program, we only want to track one discrete gesture from the database, so we'll load it by name
            foreach (Gesture gesture in database.AvailableGestures)
            {
                if (gesture.Name.Equals(this.handsLeftGestureName))//moje gesty!
                {
                    this.vgbFrameSource.AddGesture(gesture);
                }
                if (gesture.Name.Equals(this.handsRightGestureName))//moje gesty
                {
                    this.vgbFrameSource.AddGesture(gesture);
                }
            ...
            if (frame != null)
            {
                ...
                if (discreteResults != null)
                {
                    ...
                    foreach (Gesture gesture in this.vgbFrameSource.Gestures)
                    {
                        ...
                        if (gesture.Name.Equals(this.handsLeftGestureName) && gesture.GestureType == GestureType.Discrete)//uzupełnić moimi nazwami
                        {
                            ...
                            if (result != null)
                            {
                                ...
                                {
                                    this.OnGestureDetected(this, new GestureEventArgs(true, result.Detected, result.Confidence, this.handsLeftGestureName));
                                }
                            }
                        }

                        if (gesture.Name.Equals(this.handsRightGestureName) && gesture.GestureType == GestureType.Discrete)//uzupełnić moimi nazwami
                        {
                            DiscreteGestureResult result = null;
                            discreteResults.TryGetValue(gesture, out result);
                            ...
                            if (result != null)
                            {
                                if (this.OnGestureDetected != null)
                                {
                                    this.OnGestureDetected(this, new GestureEventArgs(true, result.Detected, result.Confidence, this.handsRightGestureName));
                                }
                            }
                        }
