...
public class KinectManager : MonoBehaviour
{
    ...
    private int bodyCount;//how many peopleyou want to track
    private Body[] bodies;
    //gesty
    private string leftHandGestureName = "Hands_Left";
    private string rightHandGestureName = "Hands_Right";
    ...
    void Start()
    {
        playerMotorScript = Player.GetComponent<PlayerMotor>();//obracanie u nas będzie skrypt odpowiedzialny za poruszanie
    ...
    private void OnGestureDetected(object sender, GestureEventArgs e, int bodyIndex)//wykrywanie gestów i poruszanie
    {
        var isDetected = e.IsBodyTrackingIdValid && e.IsGestureDetected;

        if (e.GestureID == leftHandGestureName)
        {
            ...
            if (e.DetectionConfidence > 0.2f)//pewność gestu
            {
                playerMotorScript.moveLeft = true;
            }
            else
            {
                playerMotorScript.moveLeft = false;
            }
        }

        if (e.GestureID == rightHandGestureName)
        {
            //NEW UI FOR GESTURE DETECTed
            //GestureTextGameObject.text = "Gesture Detected: " + isDetected;
            //StringBuilder text = new StringBuilder(string.Format("Gesture Detected? {0}\n", isDetected));
            ...
            {
                playerMotorScript.moveRight = true;
            }
            else
            {
                playerMotorScript.moveRight = false;
            }
        }
